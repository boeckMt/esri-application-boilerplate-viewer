 ...
 */
define(["dojo/Evented", "dojo/_base/declare", "dojo/_base/kernel", "dojo/_base/array", "dojo/_base/lang", "dojo/dom-class", "dojo/Deferred", "dojo/promise/all", "esri/arcgis/utils", "esri/urlUtils", "esri/request", "esri/config", "esri/lang", "esri/IdentityManager", "esri/arcgis/Portal", "esri/arcgis/OAuthInfo", "esri/tasks/GeometryService", "config/defaults", "extras/xml_config"], function (
    Evented,
    declare,
    kernel,
    array,
    ...
    return declare([Evented], {
        ...
        _init: function () {
            ...
            //'config' in paramItems fügt DRLconfig zu configObject hinzu!
            paramItems = ['webmap', 'appid', 'group', 'oauthappid', 'config'];
        ...
        _queryDisplayItem: function () {
            ...
            if (this.templateConfig.queryForWebmap) {
                if (this.config.webmap){
                    if (typeof this.config.webmap === "string"){
                        ...
                        var _itemInfo = this.config.itemInfo;
                        this.config.itemInfo = _itemInfo;
                        deferred.resolve(_itemInfo);
                    }
                } else {
        ...
        _queryApplicationConfiguration: function () {
            ...
            //------------------------------------------------------------------
            var DLRrequestType, DLRconfigURL;
            var DLRconfig = this.config.config;
            if(DLRconfig){
                DLRrequestType = DLRconfig.split(".")[1];
                DLRconfigURL = DLRconfig.split(".")[0]; 
                DLRconfigURL = this.config.appURL + '/configFiles/' + DLRconfig;
            }
            //------------------------------------------------------------------
            ...
            // Config from own WebApp
            //--------------------------------------------------------------
            else if(DLRconfig){
                    if(DLRrequestType == "json"){
                            esriRequest({
                                url: DLRconfigURL,
                                content: {"f": "json"},
                                ...
                                var response = {};
                                response.item = res.values.webmap.item;
                                response.itemData = res.values.webmap.itemData;
                                response.itemData.values = res.values; 
                                ...
                                console.log("get JSON WebMap:" + error);
                                deferred.reject(error);
                            });
                        ...
                        esriRequest({
                                ...
                                var config_data = xmlConfig.parseXML(res);
                                var response = {}; 
                                
                                response.item = config_data.webmap.item;
                                response.itemData = config_data.webmap.itemData;
                                response.itemData.values = config_data;
